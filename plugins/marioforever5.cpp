...
class PlugMarioForever5 final : public plug::PlugBase {
private:
...
public:
    PlugMarioForever5() {
        name = "Mario Forever 5.0";
        cmdline_append =
            string(" /SF \"") + string(ofs::get_cwd()) + "\\Mario Forever 5.0.exe\" /SO94208";
    ...
    static of::optional<PlugMarioForever5*> on_plugin_check() {
        if (mem::exe_name == "stdrt.exe" &&
            ofs::file_exists(string(ofs::get_cwd()) + "\\Mario Forever 5.0.exe"))
            return new PlugMarioForever5;
        return {};
    }
};

PLUG_REG(PlugMarioForever5);
